...
2.** DATE_FORMAT 함수로 출고일자 형식 변환**
   - MySQL에서 **DATE_FORMAT(OUT_DATE, '%Y-%m-%d')**을 사용하여 YYYY-MM-DD 형식으로 변환
...
* **_교훈_**  
   - 날짜 비교 시 NULL 값 처리를 놓치지 말자.... 그리고 DATE_FORMAT 함수. 오라클과 헷갈리지 말자. 클일이다 머리가 뒤죽박죽이라~~ 환장하겠다
...
## **Tip**
> **날짜 형식 변환**
>    - 날짜를 원하는 형식으로 변환하려면 **DATE_FORMAT(`OUT_DATE`, '%Y-%m-%d')**을 사용하자
{: .prompt-tip }
...
> - **날짜 비교*(ORACLE vs MySQL)**
    ...
    | 날짜 형식 변환      | **TO_CHAR(날짜, '형식')**             | **DATE_FORMAT(날짜, '형식')**      |
